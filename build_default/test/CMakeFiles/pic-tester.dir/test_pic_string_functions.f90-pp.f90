# 1 "/home/runner/work/pic/pic/test/test_pic_string_functions.f90"
# 1 "<built-in>"
# 1 "<command-line>"
...
contains
   ...
   subroutine collect_string_function_tests(testsuite)
      !> Collection of tests
   ...
   pure function reference_slice(string, first, last, stride) result(sliced_string)
      ...
      if (stride_ < 0) then
         last_ = min(max(pic_optional(last, 1), 1), len(string) + 1)
         first_ = min(max(pic_optional(first, len(string)), 0), len(string))
      else
         first_ = min(max(pic_optional(first, 1), 1), len(string) + 1)
         last_ = min(max(pic_optional(last, len(string)), 0), len(string))
      end if
