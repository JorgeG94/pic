...
contains
   ...
   pure type(string_type) function join_string(strings, separator)
      ...
      ! leave the result unallocated, so join returned an empty string. This
      ! was the only `move` call in the library, so nothing else was exposed
      ! to it. Defined assignment of a character to a string_type is used
      ! everywhere and behaves on every target compiler; the extra copy of an
      ! already-built buffer is not measurable here.
      join_string = joined
